...
export async function enhanceCodeBlocks(
  ...
  const blocks = Array.from(root.querySelectorAll<HTMLPreElement>('pre:not([data-code-enhanced])'));
  ...
  await Promise.all(
    blocks.map(async (source) => {
      ...
      wrapper.append(codeToolbar(wrapper.dataset.language, code, clipboard, timeouts), rendered);
    }),
  );
