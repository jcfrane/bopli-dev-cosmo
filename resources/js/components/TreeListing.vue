...
watch(
  ...
  () => {
    openDirectories.value = new Set(
      props.groups.map((group) => group.name).filter((name) => openDirectories.value.has(name)),
    );

    if (openDirectories.value.size === 0) {
      openDirectories.value = new Set(props.groups.map((group) => group.name));
    }
  },
);
...
<template>
  <div class="tree-listing">
    ...
    <template v-for="(group, groupIndex) in groups" :key="group.name">
      ...
      >
        <span class="tree-glyph">{{ groupIndex === groups.length - 1 ? '└── ' : '├── ' }}</span>
        <span class="tree-name">
          <span class="tree-caret">{{ openDirectories.has(group.name) ? '▾' : '▸' }}</span
          >{{ group.name }}/
        </span>
        <span class="tree-meta"
          >{{ group.entries.length }} {{ group.entries.length === 1 ? 'item' : 'items' }}</span
        >
      </button>
      ...
      <template v-if="openDirectories.has(group.name)">
        ...
        >
          <span class="tree-glyph"
            ...
            }}{{ entryIndex === group.entries.length - 1 ? '└── ' : '├── ' }}</span
          >
