...
All Vue components use `<script setup lang="ts">`. Each template keeps one typed `define*Template()` authoring declaration in that block. `npm run types` derives the ignored `resources/js/.bopli/types.d.ts` file from `package.json` settings and those declarations; `resources/js/types.ts` contains only theme-specific helpers. `types:check` regenerates the contracts and runs strict `vue-tsc`, while the CLI removes authoring-only calls before building self-contained ESM, CSS, an artifact inventory, and protocol-v1 `theme.json` with runtime ABI v1 to ignored `dist/`. Tagged releases use the pinned toolkit workflow to produce a downloadable GitHub Actions ZIP artifact. A landlord uploads that ZIP through Bopli's **Upload release** form, which verifies, publishes, registers, and promotes the release.
...
- **Template metadata** lives in one top-level typed `define*Template()` call inside the Vue file. Generic Entries declare `fields`; native Blog templates use their dedicated helpers. Page and Entry kinds are required and each has one default; Blog archive/post support is optional and paired. Companion files, custom metadata blocks, and slot declarations are not part of protocol v1.
