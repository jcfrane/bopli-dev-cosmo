...
- **Template metadata** lives in a `<bopli lang="json">` custom block. Generic Entries declare `fields`; Page and native Blog templates only need display/default metadata. Page and Entry kinds are required and each has one default; Blog archive/post support is optional and paired. Slot declarations are not part of protocol v1.
...
Every template receives resolved tenant `settings` alongside its normal public props. The shared layout applies the configured accent, toggle visibility, footer text, and optional author portrait. Home uses `useBopliQuery<BopliBlogPostSummary>()` for featured and recent posts; native templates use `BopliBlogIndexProps` and `BopliBlogPostProps`. No component imports or depends on admin application code.
